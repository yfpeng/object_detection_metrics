from deprecated.utils import load_data, load_data_coco
import math
...
def test_load_data(tests_dir):
    ...
    assert gt_bb[-1].image == '2007_001416'
    assert gt_bb[-1].category == 'cup'
    assert gt_bb[-1].score == 1

    assert len(pd_bb) == 494
    assert pd_bb[-1].image == '2007_001416'
    assert pd_bb[-1].category == 'chair'
...
def test_load_data_coco(tests_dir):
    ...
    assert len(gt_bb) == 686
    assert gt_bb[-1].image == '2007_001416.txt.jpg'
    assert gt_bb[-1].category == 'cup'
    assert gt_bb[-1].score is None

    assert len(pd_bb) == 494
    assert pd_bb[-1].image == '2007_001416.txt.jpg'
    assert pd_bb[-1].category == 'chair'
    assert pd_bb[-1].score == 0.644847
    assert math.isclose(pd_bb[-1].score, 0.644847, rel_tol=1e-3)
